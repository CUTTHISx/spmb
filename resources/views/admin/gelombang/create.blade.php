...
@section('title', 'Tambah Gelombang - PPDB Online')

@section('content')
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-900">Tambah Gelombang</h1>
        <p class="text-gray-600">Tambah gelombang pendaftaran baru</p>
    </div>

    <div class="bg-white rounded-lg shadow-sm p-6">
        <form action="/admin/gelombang" method="POST">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <input type="date" name="tgl_mulai" class="w-full border border-gray-300 rounded-lg px-3 py-2" required>
                @error('tgl_mulai')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="date" name="tgl_selesai" class="w-full border border-gray-300 rounded-lg px-3 py-2" required>
                @error('tgl_selesai')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
